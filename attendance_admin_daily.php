...
<body>
<form name="myform" action="admin_attendance_manager_daily.php" method="get">
...
$query = "SELECT persons.Username, fname, lname, check_in,DAY(check_in) ,check_out, TIME_FORMAT(attendance.check_in, '%H:%i'), TIME_FORMAT(attendance.check_out, '%H:%i') FROM persons, attendance 
                  WHERE persons.Username = attendance.username AND attendance.username IS NOT NULL AND DATE(check_in) = '$date' ORDER BY attendance.username, check_in";
